...
function PriceDisplay({ item, mode }: { item: EquipmentItem; mode: 'in_studio' | 'out_of_studio' }) {
  ...
  if (item.included && mode === 'in_studio') {
    return <span className="text-brand-green text-sm font-medium">Included</span>;
  }
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {bundles.map((item) => (
            <div
              ...
              className="rounded-xl border border-brand-border bg-brand-card p-6 transition-colors hover:border-brand-accent/30"
            >
              <h4 className="font-semibold text-brand-white">{item.name}</h4>
              {item.description && (
                <p className="mt-1 text-sm text-brand-muted">
                  {item.description}
                </p>
      ...
      <div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {alacarte.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between rounded-lg border border-brand-border bg-brand-card px-5 py-4 transition-colors hover:border-brand-accent/30"
            >
              <div>
